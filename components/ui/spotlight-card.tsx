...
interface GlowCardProps {
  ...
  /** When true, no preset size/aspect-ratio classes are applied — parent controls sizing. */
  customSize?: boolean;
  /** When true, the component's default visual chrome (padding, grid, radius, shadow) is skipped. */
  bare?: boolean;
  /** Corner radius in px for the pseudo-element glow border. Match to the card's actual border-radius. */
  radius?: number;
}
...
const glowColorMap: Record<GlowColor, { base: number; spread: number; saturation: number }> = {
  ...
  // LUMORA-specific warm gold. Narrow spread keeps hue inside the champagne→gold band.
  gold:   { base: 43,  spread: 20,  saturation: 85 },
};
...
export const GlowCard: React.FC<GlowCardProps> = ({
  ...
  // Injected once per mount — the pseudo-element glow rules can't live in Tailwind.
  ...
  return (
    <>
      ...
      <div
        ...
        style={inlineStyles}
        className={cn(
          // Size defaults — skipped when customSize or bare
          !customSize && !bare && sizeMap[size],
          !customSize && !bare && 'aspect-[3/4]',
          // Visual chrome — skipped when bare
          !bare && 'rounded-2xl grid grid-rows-[1fr_auto] shadow-[0_1rem_2rem_-1rem_black] p-4 gap-4 backdrop-blur-[5px]',
          className,
        )}
      >
